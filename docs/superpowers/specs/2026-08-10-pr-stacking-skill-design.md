...
- **Tool selection rule** (deterministic):
  1. If the user names a tool, honor it.
  2. Else if graphite is detected (`.git/refs/graphite` exists, or `gt log`
     succeeds), use `gt`.
...
Add a minimal one-way guard at the top of the `commit` workflow (no rewrite):

> Before committing, check for a stacking repo — graphite init present
> (`.git/refs/graphite`), or an active gh-stack (`gh stack view` succeeds on a
> tracked stack). If found, route the commit through the stacking tool (invoke
> `pr-stacking`) rather than raw `git commit` — untracked commits desync the stack.
> Atomic grouping and conventional-message logic still apply; they just route
> through `gt create`/`gt modify` or `gh stack add`.
