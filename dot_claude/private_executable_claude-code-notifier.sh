#!/bin/bash
# ABOUTME: Displays system notifications when Claude Code triggers hooks.
# ABOUTME: Supports macOS (terminal-notifier), Linux (notify-send), and Windows (PowerShell toast).
...
# Fallback if jq is not available or parsing fails
if [ $? -ne 0 ] || [ "$message" = "null" ]; then
  message="Claude Code Notification"
fi

# Customize message and sound based on event type
# Sounds: Glass (task done), Funk (needs attention), Submarine (session events)
sound="default"
case "$hook_event" in
  "SessionStart")
  ...
  "Stop")
    ...
    ;;
  "Notification")
    # Keep the original message from Claude - this means attention is needed
    sound="Funk"
    ;;
  *)
...
case "$(uname -s)" in
  Darwin*)
    # macOS - use terminal-notifier with distinct sounds per event type
