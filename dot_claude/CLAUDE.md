...
- NEVER make code changes unrelated to your current task — but test "unrelated" rather than assuming it. Ask: did this gap cause the bug you just fixed, or would closing it have caught it? If either is yes it belongs in this change; fix it, in its own commit. If both are no it is genuinely unrelated; file an issue instead and say so in the PR. "I noticed it while working on X" is not a relationship; being upstream or downstream of X is. Filing is the exception and needs a reason — a backlog that grows every time a bug is fixed means this test isn't being applied.
